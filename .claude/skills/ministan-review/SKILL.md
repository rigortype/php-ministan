...
| レイヤー | 問い | レンズ | いつ回すか |
|---|---|---|---|
| **L1 真**（正しさ・事実整合・公正） | 書いてあることは**正しいか／世界と整合するか** | ② 型理論・⑥ PHPStan フィデリティ・⑫ 仕様サブセット性・⑬ 実装無矛盾・④ ドメイン著者(Psalm/Matt Brown)・⑪ 別言語×型システム(mametter) | 技術的内容を書いた／変えた直後・新章 |
...
- **L1 真** ＝ 本書が*外の世界*（型理論・実 PHPStan・Psalm/型システム文献）について嘘を
  ...
  ⑥ が*本文の主張*の真偽を、⑫⑬ が*ministan の実体（本文＋実装）*を **PHPStan の仕様／実装**と
  突き合わせる（後述「⑫⑬ との分業」）。⑫⑬ は **CONTRADICTION（真の矛盾＝直す）/ SUBSET（欠如＝
  不問）/ MODERN（PHP 8.3+ ゆえの近代化＝不問）** の三分法で報告する。
...
| 12 | PHPStan 仕様サブセット性（spec-subset） | **L1 真** | ministan の実体が PHPStan の**仕様（ドキュメント）**の正当なサブセットか（矛盾だけ拾う） | `book/ja/.reviews/_spec-subset-review.md` |
| 13 | phpstan-src 実装無矛盾（src-consistency） | **L1 真** | ministan の設計/型代数/レベル分けが phpstan-src の**実装**と矛盾しないか | `book/ja/.reviews/_src-consistency-review.md` |
...
- **読む対象**：基礎編 `book/ja/wonderland/00-overview.md`〜`09-tooling.md`、
  応用編 `book/ja/looking-glass/01-*`〜（依頼に応じて）。`README.md` 等の総説があれば併せて。
...
- **⑫⑬ との分業**：⑥ は*本文に明示された主張*「実 PHPStan では…」の真偽（**本文 → 実態**）。⑫ は
  ministan の*実体*が**仕様**の正当なサブセットか、⑬ は *実体*が**実装**と矛盾しないか（**実体 → 照合**で、
  本文が何も主張していない**暗黙の乖離も拾う**）。挙動・設計の実体ベースの指摘は ⑫⑬ に寄せ、⑥ は
  *書かれた文言*の検証に集中する。三者で同じ穴に当たったら**収束指摘＝本物**として優先。
...
### レンズ 12：PHPStan 仕様サブセット性（spec-subset）〔L1 真〕
- ペルソナ：実 PHPStan の**公開仕様（ドキュメント）**を隅々まで知る査読者。ministan が掲げる概念・
  ルール・レベル・型操作・narrowing・PHPDoc 解釈が、PHPStan の **documented spec の正当なサブセット**に
  なっているか（＝仕様が ¬X と定めることを ministan が X とやっていないか）だけを検証する。**欠落の指摘は
  しない。** 反対に、仕様には無いのに ministan が**勝手に足した**挙動（サブセットからのはみ出し）は突く。
- **読む対象（一次情報＝仕様）**：`/Users/megurine/repo/site/phpstan`（＝`phpstan/phpstan` リポジトリ。
  **read-only・絶対編集しない**）の：
  - `website/src/user-guide/*.md`（`getting-started`・`rule-levels`・`troubleshooting-types`・
    `ignoring-errors` 等＝何をどのレベルで検出するかの公式仕様）
  - `website/src/developing-extensions/*.md`（`type-system`・`scope`・`trinary-logic` 等＝内部モデルの公式説明）
  - `website/src/_posts/*.md`（generics・union/intersection・list 型 等の公式解説）
  - `website/errors/*.md`（1000+ のエラー識別子定義＝「PHPStan がそのケースで何を言うか」の事典）
  - `website/src/config-reference.md`
- **照合対象（ministan 側）**：本書 `book/ja/` ＋ **ministan の実体** `dev/src/`・`impls/`（事実検証系レンズ
  なので実装を見てよい。再現性・読者レンズと違い「答え」を開いて構わない）。
- **報告の三分法（各項に必ずラベル）**：
  - **(a) CONTRADICTION（真の矛盾＝直す）**：仕様が明確に定める挙動に **ministan が反する**（サブセットでも
    縮退でも近代化でもない）。例：仕様が「level N で必ず出す」と書いたエラーを本書/実装が「PHPStan でもそうだ」
    という顔で握りつぶす・別レベルに置く／仕様の narrowing と逆の結論を「これが PHPStan の挙動」と断言、等。
  - **(b) SUBSET（欠如＝所見ではない）**：仕様にあるが ministan が*持たない*だけ。**指摘しない**（本書の軸）。
  - **(c) MODERN（近代化＝所見ではない・むしろ良）**：PHP バージョン差ゆえに ministan の方がモダン／簡潔（下記）。
- **PHP バージョン差の許容（最重要・このレンズ固有）**：PHPStan は PHP 7.x 以降の広い互換と歴史的経緯を
  背負う。ministan は **PHP 8.3+ ／ nikic/php-parser** 前提なので、同じ概念を `match`・enum・readonly・
  first-class callable・型付きプロパティ等で**よりモダン・コンパクトに実装**してよい。これは**矛盾ではない**。
  「PHPStan のコードはこう書く、ministan は違う」という*書き方の違い*は突かない。**意味（観測される型判定・
  エラーの有無）が仕様のサブセットとして整合していれば合格**。
- **禁じ手**：機能パリティ要求（「仕様に X がある→足せ」）、non-rejecting の否定（「もっと検出せよ」）、
  実装スタイル差での難癖。突くのは**「仕様が ¬X と定めるのに ministan が X」「仕様に無いのに ministan が
  勝手に足した」という真の不整合だけ**。
- 出力 `book/ja/.reviews/_spec-subset-review.md`。各項に CONTRADICTION / SUBSET / MODERN のラベル、
  仕様側の出典（ファイル＋見出し）・ministan 側の該当箇所（本文 or `dev/src` 行）を併記。

### レンズ 13：phpstan-src 実装無矛盾（src-consistency）〔L1 真〕
- ペルソナ：実 PHPStan の**実装**を読める査読者。ministan の**設計・型代数・narrowing・レベル分け・
  アーキテクチャ**が phpstan-src の実装と**矛盾しない**か（サブセットとして筋が通るか）を検証する。
  ⑫ が*仕様（ドキュメント）*との照合なのに対し、⑬ は*ソース実装*との照合。
- **読む対象（一次情報＝実装）**：`/Users/megurine/repo/php/phpstan-src`（**read-only・絶対編集しない**）の：
  - `src/Analyser/`（`Scope`・`NodeScopeResolver`・`TypeSpecifier`＝スコープと narrowing の実装）
  - `src/Type/`（型代数＝union/intersection・部分型・`accepts`・`TypeCombinator`・`*Type` クラス群）
  - `src/Rules/`（各ルールの実体）／`src/Reflection/`
  - `conf/config.levelN.neon`（level 0–10 のルール割り当て＝何レベルで何が有効か）
- **照合対象（ministan 側）**：本書 `book/ja/` ＋ `dev/src/`・`impls/`。とくに ministan の `Type` 階層・
  `Scope`・`TypeSpecifier`／narrowing／level ゲートを phpstan-src の対応物と突き合わせる。
- **報告の三分法（⑫ と同じ）**：(a) CONTRADICTION（実装が定める意味論に ministan が反する）だけ直す。
  (b) SUBSET（実装にあるが ministan が持たない＝欠如は指摘しない）。(c) MODERN（PHP 8.3+ ゆえの近代的・
  簡潔な実装＝許容、むしろ良）。
- **典型の矛盾候補（探す角度）**：union/部分型の **`accepts` 規則**が逆、narrowing の方向（肯定枝/否定枝）が逆、
  `mixed` の縮退規則が phpstan-src と相反、level の割り当てが「PHPStan ではこのレベル」と食い違うのに本書が
  そう断言、`TypeCombinator` 的な正規化（union のフラット化・部分型吸収）の結論が相反、等 ―― **意味が反する
  ものだけ**。実装の**書き方・クラス分割・最適化の差は不問**。
- **PHP バージョン差・近代化の許容（⑫ と同じ）**：phpstan-src の互換・歴史的回避コードを ministan が
  持たない／モダンに畳んでいるのは矛盾でない。**観測挙動が実装のサブセットとして整合していれば合格**。
- **禁じ手**：機能パリティ要求、non-rejecting の否定、実装スタイル・性能最適化差での難癖。
- phpstan-src は**絶対に編集しない**。出力 `book/ja/.reviews/_src-consistency-review.md`。各項に
  CONTRADICTION / SUBSET / MODERN のラベル、phpstan-src 側の出典（ファイル＋クラス/メソッド）・
  ministan 側の該当箇所を併記。

## 英語版モード（book/en/）— English read-feel battery

対象が**英語版エディション `book/en/`**（日本語版 `book/ja/` の transcreation）のときに回す追加レンズ群。
**基本運用：`book/en/` の各章を書くたびに回す**（ユーザー指定 2026-06）。最重視は **英語の技術書としての
読み味（read-feel）**＝ネイティブの良く編集された英語技術書として読めるか。transcreation ゆえの**翻訳臭
（倒置・埋もれ主語・直訳イディオム・約物）**を捕まえるのがこのモードの肝。詳細な英語版の方針・用語・約物は
**`book/en/STYLE.md`（英語版の正典）**。

- **言語非依存レンズはそのまま流用**：① 再現性（**英語本文**だけから再実装）・⑥ フィデリティ・⑫ 仕様サブ
  セット・⑬ 実装無矛盾 は、コード/仕様/実装が言語非依存なので book/en にもそのまま効く（読む本文を
  `book/en/` に差し替えるだけ）。
- **③ 編集・⑤ 校正は英語版では下の ⑭⑮ に置換**（⑤ 日本語校正は英語版では使わない）。
- 出力ノートは **`book/en/.reviews/_<lens>-review.md`**（gitignore 済 ＝ `book/*/.reviews/`）。
- 共通の約束（軸を壊さない・重大度・「答えを開かない」）は L1〜L4 と同じ。英語版固有で必ず渡す：
  **Carroll モチーフ（Wonderland/Looking-Glass）は意図的**／**transcreation なので逐語の不自然さを拾う**／
  **収束指摘（≥2 が独立に同箇所）＝本物**を最優先。

| # | レンズ | レイヤー | 主眼 | 出力ノート |
|---|---|---|---|---|
| 14 | 米ベテラン技術書編集者（1990s–2010s） | **L4 整**（＋read-feel） | line-edit の超ベテラン。経済性・signposting・promise→payoff・能動態・倒置/埋もれ主語・antecedent・rhythm | `_en-veteran-editor-review.md` |
| 15 | モダン技術書編集者（2016〜） | **L3 読/L4 整** | screen-first/skim・voice の温度・scannability・翻訳臭・Carroll が 2020s 英語 dev に効くか | `_en-modern-editor-review.md` |
| 16 | 仏 FP 研究者（ML/OCaml/Coq） | **L1 真**（＋国際読者の read-feel） | 非ネイティブ＝国際読者 proxy（曖昧さ）＋型理論健全性（subtyping/三値/lattice/gradual） | `_en-fp-researcher-review.md` |

### レンズ 14：米ベテラン技術書編集者〔read-feel 最優先・L4 整〕
- ペルソナ：1990s–2010s の名 programming 書（O'Reilly / Addison-Wesley / Pragmatic 系）を支えた
  developmental/line editor。**技術者ではない**が長年の手練れ。誇張を嫌う。
- 主眼：経済性・明快さ・論理の流れと signposting・**promise→payoff** 構造・throat-clearing/hedging/filler の
  削り・能動態と強い主語・並列・用語の一貫・**antecedent（"this/it/that" の指す先）**・rhythm・「一文が
  背負いすぎ」。各段落が場所を稼いでいるか、現場プログラマが滑らかに学べるか。
- 突く：倒置・埋もれ主語・直訳された内的独白・翻訳臭。**具体引用＋具体的な書き換え案**。重大度 High/Med/Low。

### レンズ 15：モダン技術書編集者〔read-feel 最優先・L3 読/L4 整〕
- ペルソナ：2016〜の良質な dev 書/オンライン講座（Manning / No Starch / Pragmatic / 一級ドキュメント）を
  多数手掛ける現役。**screen-first/skim** 読者・GitHub/モバイル前提。
- 主眼：精度を保った温かく直接的な voice・scannability（見出し/箇条書き/prose-to-code 比・視線の落ち所）・
  momentum と注意予算・clean な code/link 整形・plain な二人称。**翻訳臭/古臭さに敏感**、文学的趣向（Carroll）が
  2020s 英語 dev に効くか twee か。読者が bounce する所を刺す。**Carroll が効くかを明示判定**。
- 突く・書き換え案・重大度は ⑭ と同様。

### レンズ 16：仏 FP 研究者〔read-feel（国際読者）＋型理論健全性・L1 真〕
- ペルソナ：フランス人の関数型言語研究者（ML/OCaml/Coq・型理論・gradual typing）。英語は母語でない
  ＝**本書の国際読者の proxy**で曖昧さに敏感。
- 主眼（二眼）：(a) **国際英語としての読み味/明快さ**（最優先）＝idiom/省略/緩い言い回しが技術的主張を
  曖昧にしないか、(b) **型理論の健全性**（subtyping `isSuperTypeOf`・三値・`mixed`/`never`＝top/bottom・
  定数型・union・narrowing・gradual typing・健全性が正確か）。比喩が cute だが技術的に誤誘導でないかを見抜く。
- **禁じ手**：形式化/証明の要求・「理論を教えよ」は不可。**より精密な一文**の提案のみ。前提を置かない軸を尊重。

**全 ⑭⑮⑯ 共通の禁じ手**：機能パリティ要求・「もっと厳密/網羅的に」・章追加要求は禁止（軸の否定）。
意図的簡略化は欠陥ではない。`dev/`・`impls/`・`examples/` は開かない（読み味レンズ＝散文だけ。⑯ の型理論
事実確認と ⑫⑬⑥ は参照可）。

### 運用メモ
- 各 book/en 章を書いたら **⑭⑮⑯ を独立 context の subagent で並列起動**（read-feel 最優先）。必要なら
  ① 再現性・⑫⑬ も英語本文/共有コードで併走。**収束指摘を優先反映**。
- 適用後の機械検証：リンク実在・約物（spaced em dash ` — `）・**引用符 curly 一括変換時は code span と
  raw-HTML 属性（`<picture>` の `src=`/`alt=` 等）を保護**（属性は straight 必須・Python で。perl 禁止）。
  図を足したら inkscape 目視。
...
1. **スコープを選ぶ**：フルサイクル／単一レイヤー（引数 `layer:真|伝|読|整` または `L1`〜`L4`）／
   単一レンズ（`copyedit` / `fidelity` / `spec-subset` / `src-consistency` / `psalm` / `mametter` /
   `reader-7a` / `book-review` / `harsh-review` 等）。
