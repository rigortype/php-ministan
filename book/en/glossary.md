...
| **`TrinaryLogic` (three-valued logic)** | Yes / Maybe / No. Ask “is a `mixed` value an `int`?” and the answer is Maybe; ask the same of an `int` and it is Yes. Making “perhaps” a first-class answer is what becomes the axis of the level system. | [Part 3](wonderland/03-type-system.md) |
| **subtype (`isSuperTypeOf`)** | Containment, reading types as sets. `int.isSuperTypeOf(42)` is Yes — every value of `42` is an `int`. Swap the operands and the answer changes: `42.isSuperTypeOf(int)` is Maybe (a value typed `int` *might* be exactly `42`). The relation is directional, not symmetric. | [Part 3](wonderland/03-type-system.md) |
...
| **constant type** | The type of a single value, like `42`, `'foo'`, or `true`. Right after `$x = 42`, the type is `42`, not `int`. That precision is where inference gets its edge. | [Part 3](wonderland/03-type-system.md) |
