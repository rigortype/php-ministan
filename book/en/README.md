...
**analyzes itself and passes** (dogfooding). By the time you’re done, you should be able to
open the real PHPStan source and think: this is just the serious version of what you built
in ministan.

## About this book

This book takes you from *using* a static analyzer to *building* one.
...
- **PHP 8.3 or newer** (the text uses 8.3 features freely) and **Composer**.
...
An analyzer that piles up “not sure, so I’ll flag it anyway” is one nobody uses. We put
**no false positives** first, and that promise echoes through every chapter — in the level
system, and in the semantics of `mixed`. More in [Part 0](wonderland/00-overview.md).
...
### ministan Through PHP’s Looking-Glass (the advanced volume) — fleshing it out
