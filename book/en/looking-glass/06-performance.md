...
The key is the **salt**. When the analyzer’s logic or level changes, the results change too, so we mix a version and the level into the salt. The “version” here is a **schema constant baked into ministan** — you bump it by hand whenever the analysis logic changes. As a result, raising the level or updating ministan **invalidates the cache automatically**. The file path is **not** part of the key — the same contents produce the same result wherever they live.
