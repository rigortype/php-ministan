...
## An anatomy of a static analyzer
...
PHP checks type declarations **at runtime** (pass a string to `int $x` and you get a
`TypeError` on that line). What a static analyzer does is find them **before** that — without
...
That is the worth of "without running it."
...
> `mixed` is the **top type** — "anything could be here" — not a switch that turns checking
> off the way TypeScript's `any` does. Low levels wave it through; high levels object even to
> `mixed` creeping in (Part 8). Choosing, by level, whether to pass "unknown" silently or to
> object to it — that is non-rejecting, made concrete.
