...
> ＊コードはライブ実装ツリー [`dev/`](../../dev) にあります（この章の到達点は `git tag part-06`）。
...
解決します（[`Parsing`](../../dev/src/Analyser/Parsing.php)）:
...
PHPStan の心臓のひとつ [`ReflectionProvider`](../../dev/src/Reflection/ReflectionProvider.php)。
二段構えです:
...
クラス／メソッド／関数はそれぞれ
[`ClassReflection`](../../dev/src/Reflection/ClassReflection.php)・
[`MethodReflection`](../../dev/src/Reflection/MethodReflection.php)・
[`FunctionReflection`](../../dev/src/Reflection/FunctionReflection.php) に包みます。型宣言を
{@see Type} に写すのは [`TypeNodeResolver`](../../dev/src/Reflection/TypeNodeResolver.php) の
...
（[`ReflectionProviderStaticAccessor`](../../dev/src/Reflection/ReflectionProviderStaticAccessor.php)）。
...
（[`CallToUndefinedMethodRule`](../../dev/src/Rules/Methods/CallToUndefinedMethodRule.php)）。
