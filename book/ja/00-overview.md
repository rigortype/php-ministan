...
> ＊コードはライブ実装ツリー [`dev/`](../../dev) で育てています。
> 各章時点のスナップショット（`impls/00-hello` 等）は、執筆後に git 履歴から機械生成します。
...
ひとつだけ、最初に約束しておくべき哲学があります（chibirigor から借ります）:
...
次の Part 1 では AST に踏み込み、最初の本物のルール —— 構文パターンに基づく検査 ——
