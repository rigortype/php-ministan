...
> ＊この章のコードはスナップショット [`impls/03-type-system`](../../impls/03-type-system) にあります（この章の到達点は `git tag part-03`）。
...
[`TrinaryLogic`](../../impls/03-type-system/src/TrinaryLogic.php)（PHPStan の `TrinaryLogic` に対応）:
...
[`SimpleTypeTrait`](../../impls/03-type-system/src/Type/SimpleTypeTrait.php) に括り出し、`accepts` は
...
[`ConstantIntegerType`](../../impls/03-type-system/src/Type/Constant/ConstantIntegerType.php) の関係づけは
集合の直感どおりです:
...
（[`TypeTest`](../../impls/03-type-system/tests/Type/TypeTest.php)）。包含の真理値表を素直に並べます:
