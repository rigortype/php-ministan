...
> ＊コードはライブ実装ツリー [`dev/`](../../dev) にあります（この章の到達点は `git tag part-03`）。
...
[`TrinaryLogic`](../../dev/src/TrinaryLogic.php)（PHPStan の `TrinaryLogic` に対応）:
...
[`Type`](../../dev/src/Type/Type.php) です。3 つの問いに答えます:
...
[`SimpleTypeTrait`](../../dev/src/Type/SimpleTypeTrait.php) に括り出し、`accepts` は
...
[`ConstantIntegerType`](../../dev/src/Type/Constant/ConstantIntegerType.php) の関係づけは
集合の直感どおりです:
...
（[`TypeTest`](../../dev/tests/Type/TypeTest.php)）。包含の真理値表を素直に並べます:
