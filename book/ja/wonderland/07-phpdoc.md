...
> ＊この章のコードはスナップショット [`impls/wonderland/07-phpdoc`](../../../impls/wonderland/07-phpdoc) にあります（この章の到達点は `git tag part-07`）。

> 参考書（任意）：型を*コードと別に*宣言する発想は Ruby の **RBS** の親戚。配列形状 `array<K, V>` は TAPL 11 章 §11.8「レコード」、`@template` の素は『しくみ』9 章「ジェネリクス」につながります。
...
> 参考書メモ：「型を *どこに* 書くか」は言語で違います —— TypeScript はコード本体（`x: number`）、
> Ruby の **RBS** は*別ファイル* `.rbs`、PHP の PHPDoc は*コメント*。場所は違っても「本体と別レイヤーに
> 型を申告し、解析器が読む」構図は同じです。型がコードに内蔵された言語を扱う『しくみ』はこの分離を
> 主題にしませんが、RBS・PHPDoc を相手にする私たちには中心テーマになります。
...
[`PhpDocTypeResolver`](../../../impls/wonderland/07-phpdoc/src/Reflection/PhpDocTypeResolver.php) の仕事です
...
[`ArrayType`](../../../impls/wonderland/07-phpdoc/src/Type/ArrayType.php) です:
