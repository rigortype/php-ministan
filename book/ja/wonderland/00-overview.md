...
> ＊この章のコードは [`impls/wonderland/00-hello`](../../../impls/wonderland/00-hello) にあります（ライブツリー `dev/` を `git tag part-00` 時点で切り出したスナップショット）。

> 参考書（任意）：本書の **non-rejecting**（働くコードを弾かない）は、健全性をあえて崩してでも偽陽性を避ける **漸進的型付け** 寄りの立場です —— 『しくみ』「おわりに」も次のフロンティアとして挙げる領域。
