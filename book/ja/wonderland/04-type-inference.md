...
> ＊この章のコードはスナップショット [`impls/wonderland/04-type-inference`](../../../impls/wonderland/04-type-inference) にあります（この章の到達点は `git tag part-04`）。
...
> 参考書メモ：この `match` の各腕は、型理論の **型付け規則** そのものです。変数を環境から引く腕は
> TAPL 9 章 Figure 9-1 の **T-Var**、リテラルや算術の腕は TAPL 8 章・『しくみ』2 章の型付け規則
> （`42 : int` のようにリテラルへ型を与える規則）に当たります。規則を一本ずつ `match` の腕に落とすと、紙の上の
> 型システムとコードが一対一に並びます。
...
- `analyse` はルールを走らせるコールバックを渡す（[`Analyser`](../../../impls/wonderland/04-type-inference/src/Analyser/Analyser.php)）
- `annotate` は型を集めるコールバックを渡す（[`AnnotateCommand`](../../../impls/wonderland/04-type-inference/src/Command/AnnotateCommand.php)）
