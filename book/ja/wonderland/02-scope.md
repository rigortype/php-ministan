...
> ＊この章のコードはスナップショット [`impls/wonderland/02-scope`](../../../impls/wonderland/02-scope) にあります（この章の到達点は `git tag part-02`）。
...
**再帰下降**に置き換えます（[`NodeScopeResolver`](../../../impls/wonderland/02-scope/src/Analyser/NodeScopeResolver.php)）。
...
- **分岐の合流は楽観的和集合**（[`Scope::mergeWith()`](../../../impls/wonderland/02-scope/src/Analyser/Scope.php)）。
