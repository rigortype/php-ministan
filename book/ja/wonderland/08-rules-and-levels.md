...
> ＊この章のコードはスナップショット [`impls/wonderland/08-rules-and-levels`](../../../impls/wonderland/08-rules-and-levels) にあります（この章の到達点は `git tag part-08`）。
...
**レベルの正体**です（[`RuleLevelHelper`](../../../impls/wonderland/08-rules-and-levels/src/Rules/RuleLevelHelper.php)）:
...
> 参考書メモ：『しくみ』7 章にも「健全な型システムが*良いプログラムを弾く*のは誤検知だ」という指摘が
> ありました。あの本は誤検知を*減らす*方向で工夫します。ministan はもう一歩進めて、「**分からない＝
> たぶん＝黙る**」（`Maybe` を高レベルまで咎めない）を仕組みの真ん中に置くことで、偽陽性をそもそも
> 生みにくくしています。

## 型を使うルール

引数の照合は関数とメソッドで共通なので
[`ArgumentTypeChecker`](../../../impls/wonderland/08-rules-and-levels/src/Rules/ArgumentTypeChecker.php) に括り出します。各実引数を
...
これを使うのが
[`FunctionCallParameterTypesRule`](../../../impls/wonderland/08-rules-and-levels/src/Rules/Functions/FunctionCallParameterTypesRule.php)・
[`MethodCallParameterTypesRule`](../../../impls/wonderland/08-rules-and-levels/src/Rules/Methods/MethodCallParameterTypesRule.php) と、
戻り値を見る
[`FunctionReturnTypeRule`](../../../impls/wonderland/08-rules-and-levels/src/Rules/Functions/FunctionReturnTypeRule.php) です。
