...
入れます（[`TypeCombinator`](../../../impls/looking-glass/07-precision/src/Type/TypeCombinator.php)）:
...
> 参考書メモ：型を部分型で並べると **束（lattice）** になります（Part 3 の型束の図）。union は
> その **join（最小上界）** で、`mixed`＝Top が何でも吸収するので `int|mixed` は `mixed` に畳まれます。
> join／meet の理論は TAPL 16 章「部分型のメタ理論」が扱います。

## `match` 式の結果型

S4 では `match` の**腕での絞り込み**はできましたが、`match` **式全体の型**は `mixed` の
ままでした。各腕の本体の型を union すれば求まります —— ただし各腕は、その条件で
絞り込んだスコープで評価します（[`Scope::matchType`](../../../impls/looking-glass/07-precision/src/Analyser/Scope.php)）:
...
（[`analyseLoopBody`](../../../impls/looking-glass/07-precision/src/Analyser/NodeScopeResolver.php)）:
...
> 参考書メモ：「型は広がる一方（単調）だから有限ステップで頭打ちになる」というこの近似は、
> プログラム解析の **抽象解釈** の **ワイドニング** 演算子の発想です。
> 不動点へ強制的に到達させる定石で、Ruby の型解析 TypeProf も同じ系譜。TAPL／『しくみ』の静的な
> 型付け規則の外にある、解析アルゴリズムの道具です。

## 名前付き引数の型照合

S5 で名前付き引数は「位置が崩れる」と見送りました。パラメータ**名**をリフレクションに
持たせれば、名前から位置を逆引きできます（[`ArgumentTypeChecker`](../../../impls/looking-glass/07-precision/src/Rules/ArgumentTypeChecker.php)）:
