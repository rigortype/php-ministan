...
**ministan Through PHP's Looking-Glass（応用編）へようこそ。** ここからが二部目です。基礎編で芯を通した
ministan —— パース → スコープ → 型推論 → 絞り込み → ルール → 報告 —— を前提に、現場で効く
精度と仕組みを足していきます（基礎編の読了を前提とします）。応用編 S1〜S7 で何をやるかは、
[まえがき](../README.md)の「二部構成の地図」を見てください。最初の一歩は、運用の入口である
**設定**です。
...
を入れ、[`ConfigurationLoader`](../../../impls/looking-glass/01-configuration/src/Configuration/ConfigurationLoader.php) で
[`Configuration`](../../../impls/looking-glass/01-configuration/src/Configuration/Configuration.php) に写します:
