...
> ＊コードはライブ実装ツリー [`dev/`](../../../dev) にあります（この章の到達点は `git tag seasoned-07`）。

ここまでで芯は通りました。本章は仕上げ——これまで「簡略化」として見送ってきた精度を
取り戻します。`match` 式の結果型、union の部分型吸収、ループの型ワイドニング、名前付き
引数の型照合。

## union は部分型を吸収する

`int|0` は `int` であるべきです。`0` は `int` の部分型だからです。`TypeCombinator::union()`
に**部分型の吸収**を入れます（[`TypeCombinator`](../../../dev/src/Type/TypeCombinator.php)）:
...
絞り込んだスコープで評価します（[`Scope::matchType`](../../../dev/src/Analyser/Scope.php)）:
...
（[`analyseLoopBody`](../../../dev/src/Analyser/NodeScopeResolver.php)）:
...
持たせれば、名前から位置を逆引きできます（[`ArgumentTypeChecker`](../../../dev/src/Rules/ArgumentTypeChecker.php)）:
...
> ここから先は本物の PHPStan のソースが一番の教科書です。
