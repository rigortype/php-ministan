...
```console
$ dev/bin/ministan analyse examples/seasoned/narrowing.php
...
```console
$ dev/bin/ministan annotate examples/seasoned/narrowing.php
...
次の S5 では、by-ref 出力引数（`preg_match($s, $m)` の `$m`）・名前付き引数・スタブを
扱います。
